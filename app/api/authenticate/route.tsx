...
export async function GET(request: NextRequest) {
  ...
  if (process.env.DEEPGRAM_ENV === "development") {
    return NextResponse.json({
      key: process.env.DEEPGRAM_API_KEY ?? "b04819f464bd604f7af845e47e51b1eb8f36a548",
    });
  }

  // gotta use the request object to invalidate the cache every request :vomit:
  const url = request.url;
  const deepgram = createClient(process.env.DEEPGRAM_API_KEY ?? "b04819f464bd604f7af845e47e51b1eb8f36a548");
  ...
  const response = NextResponse.json({ deepgram, url });
